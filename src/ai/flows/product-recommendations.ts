...
/**
 * @fileOverview Recommends products to the user based on chat history and optionally a current product view.
 *
...
const GetProductRecommendationsInputSchema = z.object({
  ...
  currentProductView: z
    .string()
    .optional()
    .describe('The name or description of the product currently being viewed (if any).'),
});
...
Based on the user's chat history{{#if currentProductView}} and the product they are currently viewing ({{{currentProductView}}}){{/if}}, you will recommend a list of related products.

Chat History:
{{{chatHistory}}}

Reasoning:
First, you must walk through your reasoning for the product recommendations, as a single paragraph. Be specific as possible, and draw connections between the chat history, {{#if currentProductView}}the current product view, {{/if}}and the recommended products. 
